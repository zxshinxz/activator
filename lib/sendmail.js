...
module.exports = function (turl) {
  ...
  return function (from, to, subject, text, html, filesToAttach, cb) {
    ...
    if (filesToAttach && filesToAttach.length > 0) {
      for (var i = 0; i < filesToAttach.length; i++) {
        attachments.push({
          filename: filesToAttach[i].filename,
          path: filesToAttach[i].fd,
          cid: "attachment" + i //same cid value as in the html img src
        })
      }
    }
