...
var async = require('async'), crypto = require('crypto'), smtp = require('./mailer'), _ = require('lodash'), mailer,
            ...
            async.waterfall([
                ...
                function (res, cb) {
                    if (!res) {
                        ...
                        code = sha1(email + new Date().toString().split("").sort(function () {
                ...
                function (res, cb) {
                    ...
                    mailer("activate", req.lang || "en_US", {
                        code: code,
                        email: email,
                        id: id,
                        request: req
    ...
    createPasswordReset = function (req, done) {
        ...
        id = (req.activator ? req.activator.id : null);
        /*
         * process:
        ...
        async.waterfall([
            ...
            function (res, cb) {
                if (!res || res.length < 1) {
                    ...
                    reset_code = sha1(email + new Date().toString().split("").sort(function () {
                        return Math.round(Math.random()) - 0.5;
                    })).substr(0, 8);
    ...
    completePasswordReset = function (req, done) {
        var reset_code = req.param("code"), id = req.param("user"), now = new Date().getTime();
        async.waterfall([
            function (cb) {
            ...
            function (res, cb) {
                if (!res) {
                    ...
                    cb("invalidresetcode", null);
                } else if (res.password_reset_time < now) {
                    cb("expiredresetcode", null);
                } else {
                    model.save(idProperty ? res[idProperty] : id, {password_reset_code: "X", password_reset_time: 0},
                        function (err) {
                            cb(err, res)
                        });
                }
...
module.exports = {
    ...
    completePasswordResetNext: function (req, res, next) {
        ...
        completePasswordReset(req, function (code, message) {
            req.activator = req.activator || {};
            _.extend(req.activator, {code: code, user: user});
            next();
        });
    },
